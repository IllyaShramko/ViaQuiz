...
export function AuthCard({
  ...
  return (
    <div className={styles['auth-page']}>
      <div className={`${styles['auth-card']} ${isWide ? styles['auth-card-wide'] : ''}`}>
        <div className={styles['auth-header-bar']}>
          <div className={styles['auth-lang-switcher']} role="group" aria-label={t('nav.language')}>
            <button
              type="button"
              className={`${styles['auth-lang-btn']} ${locale === 'uk' ? styles['is-active'] : ''}`}
              onClick={() => setLocale('uk')}
              title="Українська"
            >
              UA
            </button>
            <span className={styles['auth-lang-divider']} aria-hidden="true">|</span>
            <button
              type="button"
              className={`${styles['auth-lang-btn']} ${locale === 'en' ? styles['is-active'] : ''}`}
